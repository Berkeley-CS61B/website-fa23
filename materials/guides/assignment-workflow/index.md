...
you will pull from the skeleton. First ensure that you are in your `sp23-s***`
...
is located at `https://github.com/Berkeley-CS61B/skeleton-sp23.git`) and copies
...
1.  Find and choose the directory of your current assignment. For example, for
    Lab 1, you would select the `lab01` directory inside your `sp23-***` repo.
...
1.  Still in Project Structure, go to the **Libraries** tab. Click the
    "**+ -> Java**" button, then navigate to `library-sp23`, select the
    folder, and click "Ok".
...
1.  Add your assignment directory using `git add`. For example, for Lab 1,
    from your repo root (`sp23-***`) you would use `git add lab01`. From the
...
1.  Open the assignment on Gradescope. Select Github, then your `sp23-s***`
...
see [https://sp23.datastructur.es/materials/guides/git/wtfs/](https://sp23.datastructur.es/materials/guides/git/wtfs/).
